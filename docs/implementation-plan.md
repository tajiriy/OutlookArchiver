...
| `Data/EmailRepository.vb` | emails/attachments テーブルのCRUD |

### NuGetパッケージ

| パッケージ | バージョン | 用途 |
|----------|-----------|------|
| `System.Data.SQLite` | 1.0.118.0 | SQLite |
...
- SQL WHERE 句ベースの検索（件名・本文・差出人・添付ファイル名）
- 検索結果のヒット箇所ハイライト
...
- 検索機能は `EmailSearchFilter` による SQL WHERE 句ベースに移行済みで FTS4 は未使用だったため、FTS4 テーブル・同期トリガー・rebuild 処理を完全に削除
- これにより取り込み時のトリガー無効化/再構築オーバーヘッドが解消
