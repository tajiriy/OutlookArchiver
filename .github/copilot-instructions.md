...
このファイルは GitHub Copilot にプロジェクトのコンテキストを提供する。
共通の開発ルールは [CONTRIBUTING.md](../CONTRIBUTING.md) も参照すること。
...
> `build.ps1` を使用すること。直接 `msbuild` コマンドは使わない。
> bash 環境の場合は `./build.sh` / `./test.sh` も利用可能。
...
- **PATCH**: `build.ps1` (または `build.sh`) 実行時に自動で +1（ビルドごとにインクリメント）
- **MINOR / MAJOR**: ユーザーの指示で上げる。または機能追加・破壊的変更を検知した場合は AI が提案する
